...
class DatePartsField extends FormComponent {
  ...
  getFormDataFromState (state) {
    ...
    return {
      [`${name}__day`]: value ? '' + moment(value).toDate().getDate() : '',
      [`${name}__month`]: value ? '' + (moment(value).toDate().getMonth() + 1) : '',
      [`${name}__year`]: value ? '' + moment(value).toDate().getFullYear() : ''
    }
  }
